...
- [`/third-degree`](./skills/engineering/third-degree/SKILL.md) — interviews you one question at a time, walking the decision tree until the spec is sharp. Each question comes with structured options and a recommended pick; anything the codebase already answers is inferred rather than asked. Front of the pipeline — invoke when the idea is fuzzy, before `/visualise-plan`.
- [`/visualise-plan`](./skills/engineering/visualise-plan/SKILL.md) — produces a single-file HTML implementation plan at `docs/plan/<slug>.html` (Botanical Almanac styling: milestones, data-flow SVG, mockups, files-touched list, key code). Invoke *after* `/third-degree` (or your own clarifying pass) but *before* any code is written — then hard-stops until you reply "approved".
